...
In this example, `sample(-10:10, 1)` generates a random number between -10 and 10, and the value is assigned to the `number` variable. Additionally, `set.seed(123)` sets the seed to 123. This ensures that when generating the random number with `sample()`, the same number is chosen every time the code is executed. Then, we check if the number is positive or not and print the corresponding message.
...
```{r}
actions <- c("Learn to program in R",
